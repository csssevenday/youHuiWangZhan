<template>
  <div class="coupon-card bg-white rounded-xl shadow-md hover:shadow-lg transition-shadow p-4 flex flex-col sm:flex-row gap-4">
    <div class="flex-shrink-0">
      <NuxtLink :to="`/stores/${coupon.storeSlug}`" class="block">
        <div class="w-16 h-16 rounded-full bg-gradient-to-br from-blue-500 to-blue-700 flex items-center justify-center text-white text-xl font-bold hover:from-blue-600 hover:to-blue-800 transition-colors">
    ...
    <div class="flex-1 min-w-0">
      <div class="flex items-center gap-2 mb-1">
        <NuxtLink :to="`/stores/${coupon.storeSlug}`" class="text-sm text-blue-600 hover:text-blue-700 font-medium">
          {{ coupon.storeName }}
        </NuxtLink>
      ...
      <p v-if="coupon.description" class="text-sm text-gray-600 mt-1 line-clamp-2">{{ coupon.description }}</p>

      <div class="flex items-center gap-3 mt-3">
        <div class="flex items-center gap-1">
          <span class="text-xs text-gray-500">{{ coupon.successRate }}% success</span>
        </div>
        <span v-if="coupon.expiryDate" class="text-xs text-gray-400">
